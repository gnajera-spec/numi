        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input type="datetime-local" required value={form.fecha_hora}
              onChange={(e) => setForm({ ...form, fecha_hora: e.target.value })} className={cls} style={sty} />
          </div>
          <div className="flex flex-col gap-1">
            ...
            <input required value={form.lugar} onChange={(e) => setForm({ ...form, lugar: e.target.value })}
              className={cls} style={sty} placeholder="Ej: Planta baja, sector logística" />
          </div>
          <div className="flex flex-col gap-1">
            ...
            <textarea required rows={3} value={form.descripcion}
              onChange={(e) => setForm({ ...form, descripcion: e.target.value })} className={cls} style={sty} />
          </div>
          <div className="border-t pt-3" style={{ borderColor: "var(--color-surface-border)" }}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <input value={form.testigo_nombre} onChange={(e) => setForm({ ...form, testigo_nombre: e.target.value })}
                className={cls} style={sty} placeholder="Nombre" />
              <input value={form.testigo_legajo} onChange={(e) => setForm({ ...form, testigo_legajo: e.target.value })}
                className={cls} style={sty} placeholder="Legajo" />
            </div>
          </div>
...
function UpdateEstadoModal({ accidente, onClose, onUpdated }: {
  accidente: AccidenteTrabajo; onClose: () => void; onUpdated: () => void;
}) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      await medicoService.updateAccidente(accidente.id, {
        estado, numero_art: numero_art || undefined,
      });
      onUpdated();
    } catch (err) {
...
export function AdminMedicoAccidentesPage() {
      ...
      <div>
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <AlertTriangle size={22} style={{ color: "var(--color-primary)" }} />
            <h1 className="text-[22px] font-bold" style={{ color: "var(--color-content-primary)" }}>Accidentes de trabajo</h1>
          </div>
                  ...
                  <div key={acc.id} className="rounded-xl border p-4"
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        ...
                        <p className="text-xs mb-1" style={{ color: "var(--color-content-secondary)" }}>
                          Lugar: {acc.lugar}
                        </p>
                      ...
                      {acc.estado !== "cerrado" && (
                        <Button variant="secondary" onClick={() => setUpdating(acc)}>
                          Actualizar
                        </Button>
                      )}
                    </div>
