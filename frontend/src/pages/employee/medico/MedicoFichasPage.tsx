...
function inputCls() { return "rounded-lg border px-3 py-2 text-sm outline-none"; }
function inputSty() { return { borderColor: "var(--color-surface-border)", color: "var(--color-content-primary)" }; }
...
function FichaTab({ userId, nombre }: { userId: string; nombre: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (msg.includes("404") || msg.includes("no encontrada")) { setFicha(null); }
      else { setError(msg || "Error al cargar ficha"); }
    ...
    return (
      <form onSubmit={handleSave} className="flex flex-col gap-3">
        ...
        <FieldRow label="Alergias (separadas por coma)" id="alg">
          <input id="alg" className={inputCls()} style={inputSty()} value={form.alergias}
        ...
        <FieldRow label="Condiciones preexistentes (separadas por coma)" id="cond">
          <input id="cond" className={inputCls()} style={inputSty()} value={form.condiciones}
            onChange={(e) => setForm({ ...form, condiciones: e.target.value })} placeholder="Hipertensión, Diabetes" />
        </FieldRow>
        <FieldRow label="Observaciones" id="obs">
          <textarea id="obs" rows={3} className={inputCls()} style={inputSty()} value={form.observaciones}
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-2 gap-3">
        {[{ label: "Grupo sanguíneo", value: ficha?.grupo_sanguineo }, { label: "Factor RH", value: ficha?.factor_rh }].map(({ label, value }) => (
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {ficha!.alergias!.map((a, i) => (
              <span key={i} className="text-xs rounded-full px-2 py-0.5" style={{ background: "#fef2f2", color: "var(--color-state-absent)" }}>{a.nombre}</span>
            ))}
          </div>
        </div>
      )}
...
function ExamenesTab({ userId }: { userId: string }) {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    setFormError(null);
    try {
      await medicoService.createExamen(userId, { tipo: form.tipo, fecha: form.fecha, resultado: form.resultado || undefined, medico_responsable: form.medico_responsable || undefined });
      setShowModal(false);
    ...
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {items.map((ex) => (
            <div key={ex.id} className="rounded-lg border px-3 py-2" style={{ background: "var(--color-surface-card)", borderColor: "var(--color-surface-border)" }}>
              <span className="text-sm font-medium" style={{ color: "var(--color-content-primary)" }}>{tipoExamenLabel[ex.tipo] ?? ex.tipo}</span>
        ...
        <Modal title="Nuevo examen" onClose={() => setShowModal(false)}>
          <form onSubmit={handleCreate} className="flex flex-col gap-3">
            ...
            <FieldRow label="Tipo *" id="et">
              <select id="et" required value={form.tipo} onChange={(e) => setForm({ ...form, tipo: e.target.value })} className={inputCls()} style={inputSty()}>
                {Object.entries(tipoExamenLabel).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </FieldRow>
            <FieldRow label="Fecha *" id="ef">
              <input id="ef" type="date" required value={form.fecha} onChange={(e) => setForm({ ...form, fecha: e.target.value })} className={inputCls()} style={inputSty()} />
            </FieldRow>
            <FieldRow label="Médico responsable" id="em">
              <input id="em" value={form.medico_responsable} onChange={(e) => setForm({ ...form, medico_responsable: e.target.value })} className={inputCls()} style={inputSty()} placeholder="Dr/a. Apellido" />
            </FieldRow>
            <FieldRow label="Resultado" id="er">
              <textarea id="er" rows={2} value={form.resultado} onChange={(e) => setForm({ ...form, resultado: e.target.value })} className={inputCls()} style={inputSty()} />
            </FieldRow>
            <div className="flex justify-end gap-2">
...
function AptitudesTab({ userId }: { userId: string }) {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    setFormError(null);
    try {
      await medicoService.createAptitud(userId, { puesto_id: form.puesto_id, estado: form.estado, restricciones: form.restricciones || undefined, fecha_emision: form.fecha_emision, fecha_vencimiento: form.fecha_vencimiento || undefined });
      setShowModal(false);
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {items.map((apt) => {
            ...
            return (
              <div key={apt.id} className="rounded-lg border px-3 py-2" style={{ background: "var(--color-surface-card)", borderColor: "var(--color-surface-border)" }}>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-sm font-medium" style={{ color: "var(--color-content-primary)" }}>{puesto?.nombre ?? apt.puesto_id}</span>
                  <span className="text-xs font-medium rounded-full px-2 py-0.5" style={{ color: badge.color, background: `${badge.color}18` }}>{badge.label}</span>
                </div>
                <p className="text-xs mt-0.5" style={{ color: "var(--color-content-secondary)" }}>
                  Emitida {formatDate(apt.fecha_emision)}{apt.fecha_vencimiento && ` · Vence ${formatDate(apt.fecha_vencimiento)}`}
                </p>
                {apt.restricciones && <p className="text-xs mt-1" style={{ color: "var(--color-state-pending)" }}>Restricciones: {apt.restricciones}</p>}
              </div>
            );
          })}
        </div>
        ...
        <Modal title="Nueva aptitud laboral" onClose={() => setShowModal(false)}>
          <form onSubmit={handleCreate} className="flex flex-col gap-3">
            ...
            <FieldRow label="Puesto *" id="ap">
              <select id="ap" required value={form.puesto_id} onChange={(e) => setForm({ ...form, puesto_id: e.target.value })} className={inputCls()} style={inputSty()}>
                <option value="">Seleccionar puesto</option>
                {puestos.map((p) => <option key={p.id} value={p.id}>{p.nombre}</option>)}
              </select>
            </FieldRow>
            <FieldRow label="Estado *" id="ae">
              <select id="ae" required value={form.estado} onChange={(e) => setForm({ ...form, estado: e.target.value })} className={inputCls()} style={inputSty()}>
            ...
            {form.estado === "apto_con_restricciones" && (
              <FieldRow label="Restricciones *" id="ar">
                <textarea id="ar" required rows={2} value={form.restricciones} onChange={(e) => setForm({ ...form, restricciones: e.target.value })} className={inputCls()} style={inputSty()} />
              </FieldRow>
            )}
            <div className="grid grid-cols-2 gap-3">
              <FieldRow label="Fecha emisión *" id="aef">
                <input id="aef" type="date" required value={form.fecha_emision} onChange={(e) => setForm({ ...form, fecha_emision: e.target.value })} className={inputCls()} style={inputSty()} />
              </FieldRow>
              <FieldRow label="Vencimiento" id="afv">
                <input id="afv" type="date" value={form.fecha_vencimiento} onChange={(e) => setForm({ ...form, fecha_vencimiento: e.target.value })} className={inputCls()} style={inputSty()} />
              </FieldRow>
            </div>
            <div className="flex justify-end gap-2">
              <Button variant="secondary" type="button" onClick={() => setShowModal(false)}>Cancelar</Button>
...
function VacunacionesTab({ userId }: { userId: string }) {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    setFormError(null);
    try {
      await medicoService.createVacunacion(userId, { vacuna: form.vacuna, fecha: form.fecha, lote: form.lote || undefined, proxima_dosis: form.proxima_dosis || undefined });
      setShowModal(false);
  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {items.map((v) => (
            <div key={v.id} className="rounded-lg border px-3 py-2" style={{ background: "var(--color-surface-card)", borderColor: "var(--color-surface-border)" }}>
              <p className="text-sm font-medium" style={{ color: "var(--color-content-primary)" }}>{v.vacuna}</p>
              <p className="text-xs mt-0.5" style={{ color: "var(--color-content-secondary)" }}>
                {formatDate(v.fecha)}{v.lote && ` · Lote: ${v.lote}`}{v.proxima_dosis && ` · Próxima dosis: ${formatDate(v.proxima_dosis)}`}
              </p>
            </div>
          ))}
        </div>
        ...
        <Modal title="Registrar vacuna" onClose={() => setShowModal(false)}>
          <form onSubmit={handleCreate} className="flex flex-col gap-3">
            ...
            <FieldRow label="Vacuna *" id="vn">
              <input id="vn" required value={form.vacuna} onChange={(e) => setForm({ ...form, vacuna: e.target.value })} className={inputCls()} style={inputSty()} placeholder="Ej: Hepatitis B" />
            </FieldRow>
            <div className="grid grid-cols-2 gap-3">
              <FieldRow label="Fecha *" id="vf">
                <input id="vf" type="date" required value={form.fecha} onChange={(e) => setForm({ ...form, fecha: e.target.value })} className={inputCls()} style={inputSty()} />
              </FieldRow>
              <FieldRow label="Lote" id="vl">
                <input id="vl" value={form.lote} onChange={(e) => setForm({ ...form, lote: e.target.value })} className={inputCls()} style={inputSty()} />
              </FieldRow>
            </div>
            <FieldRow label="Próxima dosis" id="vpd">
              <input id="vpd" type="date" value={form.proxima_dosis} onChange={(e) => setForm({ ...form, proxima_dosis: e.target.value })} className={inputCls()} style={inputSty()} />
            </FieldRow>
            <div className="flex justify-end gap-2">
              <Button variant="secondary" type="button" onClick={() => setShowModal(false)}>Cancelar</Button>
              <Button type="submit" loading={saving}>Registrar</Button>
...
function FichaDetailModal({ item, onClose }: { item: FichaMedicaSummary; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto" style={{ background: "rgba(0,0,0,0.4)" }}>
      <div className="w-full max-w-2xl rounded-xl border mt-8 mb-8 flex flex-col" style={{ background: "var(--color-surface-card)", borderColor: "var(--color-surface-border)" }}>
        ...
        <div className="flex gap-1 px-6 pt-4">
          {detailTabs.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setActiveTab(id)}
              ...
              style={activeTab === id ? { color: "var(--color-primary)", background: "var(--color-surface-empty)" } : { color: "var(--color-content-secondary)" }}>
              <Icon size={13} /> {label}
            </button>
          ))}
        </div>
...
export function MedicoFichasPage() {
    ...
    <MedicoLayout>
      <div>
        <div className="mb-6 flex items-center gap-3">
          <Stethoscope size={22} style={{ color: "#1a7a45" }} />
          <h1 className="text-2xl font-bold" style={{ color: "var(--color-content-primary)" }}>Fichas médicas</h1>
        </div>
          ...
          <>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {item.grupo_sanguineo && (
                      <span className="text-xs font-mono font-semibold rounded px-1.5 py-0.5" style={{ background: "#fef2f2", color: "var(--color-state-absent)" }}>{item.grupo_sanguineo}</span>
                    )}
            ...
            {total > PAGE_SIZE && (
              <div className="flex items-center justify-between mt-4">
                <p className="text-xs" style={{ color: "var(--color-content-secondary)" }}>{total} colaboradores</p>
